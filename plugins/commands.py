...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [
            [
                InlineKeyboardButton('🔔 Updates 🤖', url='https://telegram.me/FilmMoraUpdate')
            ],
            [
                InlineKeyboardButton('🙆🏻 Help 🦾', url=f"https://t.me/{BOT_USERNAME}?start=help"),
            ],[
            InlineKeyboardButton('🙋 Support Group 🙋', url='https://telegram.me/FilmMoraSupport')
            ],
            [
        ...
        await message.reply(script.START_TXT.format(message.from_user.mention if message.from_user else message.chat.title, BOT_USERNAME, temp.B_NAME), reply_markup=reply_markup)
    ...
    if len(message.command) != 2:
        ...
        await message.reply_photo(
            ...
            caption=script.START_TXT.format(message.from_user.mention, BOT_USERNAME, temp.B_NAME),
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        try:
            ...
            logger.error("Pʟᴇᴀsᴇ Mᴀᴋᴇ Sᴜʀᴇ, Yᴏᴜ Mᴀᴋᴇᴅ Bᴏᴛ's Aᴅᴍɪɴ Iɴ Yᴏᴜʀ Cʜᴀɴɴᴇʟ...!")
            return
        btn = [
            [
...
async def delete_all_index_confirm(bot, message):
    ...
    await message.answer('♥️ Thank You @FilmMoraUpdate ♥️')
